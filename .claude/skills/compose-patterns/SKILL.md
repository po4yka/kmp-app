...
- **Routes are not `sealed`** — Kotlin `sealed` cannot span Gradle modules. Use a plain `interface Route : NavKey` + explicit polymorphic registration in `shared/…/AppNavigation.kt`'s `SavedStateConfiguration`.
...
- Resources are **module-scoped**. Put feature-specific strings/drawables/fonts in that feature's `:impl` module: `feature/<name>/impl/src/commonMain/composeResources/`. App-global resources live in `:shared/src/commonMain/composeResources/`.
