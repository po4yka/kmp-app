...
This project's theme (`composeApp/src/commonMain/kotlin/com/po4yka/app/ui/theme/`) is already wired for this system — `Color.kt`, `Type.kt`, and `Tokens.kt` hold the palette, typography, and non-Material tokens. Fonts live in `composeApp/src/commonMain/composeResources/font/`.
...
Spacing is the primary tool for communicating relationships. Use `IndustrialTokens.Spacing` from `Tokens.kt`.
...
1. **Confirm fonts are bundled** — check `composeApp/src/commonMain/composeResources/font/` has `space_grotesk.ttf`, `space_mono_regular.ttf`, `space_mono_bold.ttf`, `doto.ttf`. All four required. If any missing, fetch them before writing theme code (see `references/compose-mapping.md`).
...
- **`references/compose-mapping.md`** — Compose Multiplatform integration: `Res.font.*` wiring, Material 3 `ColorScheme` slot mapping, `IndustrialTokens` usage for values outside M3, dark/light switching.
