...
All values in Compose primitives. The project theme already wires these — see `Color.kt`, `Type.kt`, `Tokens.kt` in `composeApp/src/commonMain/kotlin/com/po4yka/app/ui/theme/`.
...
**Why these fonts:** Doto is a variable dot-matrix typeface — instrument-panel aesthetic. Space Grotesk + Space Mono share the same DNA (geometric sans + monospaced companion), giving a coherent typographic voice.
